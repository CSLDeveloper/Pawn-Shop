#!/usr/bin/env python3
"""
Golden Ball Pawn Shop — Web Application
...
APP_PASSWORD = os.environ.get("APP_PASSWORD", "")   # Set this on Render for security

CATEGORIES = [
    "Gold — Buying", "Gold — Selling",
    "Silver — Buying", "Silver — Selling",
    "Jewelry", "Ladies Rings", "Men's Rings", "Watches",
...
def api_get_settings():
    if auth_required():
        return jsonify({"error": "unauthorized"}), 401
    keys = ["shop_name", "shop_phone", "email_from", "email_smtp",
            "email_port", "twilio_from"]
